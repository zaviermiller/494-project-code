...
class GameStateRunning(GameState):
    ...
    def draw_focused_block(self):
        ...
        if block:
            ...
            pyglet.graphics.draw(24, GL.GL_QUADS, ('v3f/stream', vertex_data))
            GL.glPolygonMode(GL.GL_FRONT_AND_BACK, GL.GL_FILL)

    def compute_fps_stats(self):
        curr_fps = pyglet.clock.get_fps()

        print(f"{round(curr_fps)}")
        # x, y, z = self.player.position
